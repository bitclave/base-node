...
class SearchRequestController(
    ...
    /**
     * Creates new request for search in the system, based on the query full text search.
     ...
     */
    @ApiOperation("Creates new request for search in the system, based on the the query full text search.\n" +
            "The API will verify that the request is cryptographically signed by the owner of the public key.",
            response = SearchRequest::class)
    @ApiResponses(value = [
        ApiResponse(code = 201, message = "Success", response = SearchRequest::class),
    ...
    fun searchRequestByQuery(
            @ApiParam("id of already created a search request. who has rtSearch tag")
            ...
            @ApiParam("where client sends full text search query string and signature of the message.", required = true)
            @RequestBody
        ...
        return accountService.accountBySigMessage(request, getStrategyType(strategy))
                ...
                .thenCompose {
                    ...
                    val result = searchRequestService.createSearchRequestByQuery(
                            id,
                            it.publicKey,
                            request.data!!,
                    ...
                    logger.error("Request: createSearchRequest owner: $owner; request: $request; error:${e}")
                    throw e
                }
    }
    ...
    fun getSearchRequests(
        ...
        return searchRequestService.getSearchRequests(id
                ?: 0, owner, getStrategyType(strategy)).exceptionally { e ->
